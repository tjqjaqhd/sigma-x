...
class AnomalyDetector:
    ...
    def __init__(
        ...
        self.history: Dict[str, Deque[float]] = collections.defaultdict(
            lambda: deque(maxlen=window)
        )

    async def scan(
        self, metric: str, value: float, ts: float
        ...
        if value > mean + self.threshold * std:
            alert = {"metric": metric, "value": value, "level": "warning", "ts": ts}
    ...
    async def notify(self, alert: Dict[str, float]) -> None:
        ...
        await self.notifier.notify(
            {"level": alert["level"], "message": msg, "ts": alert["ts"]}
        )
