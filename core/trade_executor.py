...
from .order_executor import OrderExecutor, OrderHandler
...
class TradeExecutor:
    ...
    def __init__(
        ...
        *,
        ...
        self.order_handler = OrderHandler(api_client=None, db_session=db_session, redis_client=self.redis)
    ...
    async def run(self, limit: Optional[int] = None) -> None:
        ...
        async def handle(price: float) -> None:
            ...
            if signal in ("BUY", "SELL") and self.risk.check(signal):
                ...
                response = await self.order_executor.execute(
                    signal,
                    price,
                    symbol=self.symbol,
                    volume=self.volume,
                ...
                # Redis 기록
                self.redis_client.set(f"order:{response['id']}", response)
