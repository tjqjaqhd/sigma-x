...
- 커밋 메세지와 내용은 한글로 해주세요
- 사용자는 현제 코드만 구현하는중이니 추가의존성설치는 requirements.txt 에 추가해주세요
- 현재 # .env.example에 명시되지 않은건 준비되지 않은거다 그리고 db도 이전꺼기 때문에 덮어씌워야해
- 사용자는 자동화 구축은 어설프게 꾸역꾸역 해뒀지만 코드와 개발은 모르니 설명은 자세하게 문서로 남겨줘야한다
- docs 에 기본적으로 있어야할 상위 문서가 없다면 완전하게 만들어주고 스팩 디렉토리도 만들어서 코드와 완벽 1:1 매칭되는 사양서를 작성해줘 기본 틀이 필요하다면 모든 정보가 담길수있는 기본 템플릿 먼저 제공해줘도 괜찮아
- 현재 목표는 위의 챠트플로우와 요구사항에 완전 부합하는 시스템을 1차 구축하는게 목표이다
